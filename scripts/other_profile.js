...
// Checks if a user is allowed to post on another user's 
function checkPrivacy() {
	if (mssg == "Not Friends") {
		showSnackbar("Become friends to post on " + user.firstname + "'s" + " wall!", "snackbar");
    } else {
    	updateStatus(currUuid);
    }
}
...
function populateProfileInfo() {
    ...
    if (info.includes("empty")) {
    	profile_interests.innerHTML = firstname + " hasn't listed any interests!";
    } else {
...
// Updates the appropriate name fields based on the current logged in
// user and other user's profile
function updateName() {
     ...
     for (var i = 0; i < name.length; ++i) {
    	 name[i].innerHTML = currUser.fullname;
     }

     profileName.innerHTML = user.fullname;

     var statusHint = document.getElementById("message");
     statusHint.placeholder = "Let " + user.firstname + " know what's on your mind."
}

// Updates the appropriate user profile image fields based on the
...
function updateProfileImg() {
     ...
     for (var i = 0; i < currUserProfileImg.length; ++i) {
          currUserProfileImg[i].src = currUser.profileimageurl;
     }
...
function updateProfileLinks() {
     ...
     for (var i = 0; i < currUserProfileLinks.length; ++i) {
         currUserProfileLinks[0].href = "/" + currUser.firstname.replace(/\s/g, "") + "." + currUser.lastname.replace(/\s/g, "") + "/" + currUser.uuid;
     }

     var navBarLink = document.getElementById("nav-bar-profile-link");
     navBarLink.href = "/" + currUser.firstname.replace(/\s/g, "") + "." + currUser.lastname.replace(/\s/g, "") + "/" + currUser.uuid;
}

// Sets up status update container for post on other user's profile
...
/*
 *  AJAX call to fetch the user's friend list
 */
...
// Displays friends in user's friend list
			...
			$.post('/fetchuser', {
				 id : request.requesterid
				    ...
				    accept.style.color = "#3CB371";
				  	accept.onclick = function(){acceptRequest(user.uuid, currUuid, request.friendrequestid);};
				    ...
				    deny.style.color = "#eb4034";
				  	deny.onclick = function(){denyRequest(user.uuid, currUuid, request.friendrequestid);};
	...
	$.post('/acceptrequest', {
		...
		if (success) {
			...
			if (newNumRequests > 0) {
				...
				if (recieved) {
					var statusbar = document.getElementById("friendship-status-bar");
				  	var status = document.getElementById("currStatus");
				  	var btn = document.getElementById("request-available");
				  	
				  	statusbar.style.backgroundColor = "#E7F5ED";
			  		statusbar.style.color = "#3CB371";
...
/*
 * AJAX call to send a friend request to another user in Pennbook
 */
function sendRequest() {
	...
	$.post('/sendrequest', {
		...
		if (typeof (response) == 'string') {
			...
			showSnackbar(response.alert, "successsnackbar");
			var btn = document.getElementById("request");
...
/*
 * AJAX call to check if a user sent a friend request to another
 */
function checkFriendRequestStatusSent() {
	var userid = uuid;
...
/*
 * AJAX call to check if a user sent a friend request to the current user
 */
function checkFriendRequestStatusRecieved() {
	var userid = currUuid;
	...
	$.post('/checkrequest', {
		...
		if (response) {
			var btn = document.getElementById("request");
			btn.innerHTML = "This User Has Sent You A Request";
			btn.disabled = true;
			recieved = true;
		}
	});
...
/*
 * Helper function to check whether two users are friends
 */
function checkFriendshipStatus() {
  	...
  	if (mssg == "Not Friends") {
  		statusbar.style.backgroundColor = "#FCE7E6";
  		statusbar.style.color = "#eb4034";
  		status.innerHTML = mssg;
  		checkFriendRequestStatusSent();
  		checkFriendRequestStatusRecieved();
  	} else {
  		statusbar.style.backgroundColor = "#E7F5ED";
  		statusbar.style.color = "#3CB371";
  		status.innerHTML = mssg;
